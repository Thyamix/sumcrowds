...
func GetTotalAndMax(festivalCode string) (int, int, error) {
	...
	err = DB.QueryRow("SELECT total FROM event WHERE id = $1", eventId).Scan(&currentTotal)
	if err != nil {
...
func IsValidEventId(eventId int64) (bool, error) {
	var result int
	err := DB.QueryRow("SELECT COUNT(1) FROM event WHERE id = $1", eventId).Scan(&result)
	if err != nil {
...
func Reset(festivalId int64) (int64, error) {
	...
	err := DB.QueryRow("SELECT id FROM event WHERE active = TRUE AND festival_id = $1", festivalId).Scan(&oldEventId)
	if err != nil {
...
func getNumberOfEventEntries(eventID int64, archived bool) (int64, error) {
	...
	var err error
	if archived {
		err = DB.QueryRow("SELECT COUNT(*) FROM archive WHERE event_id = $1", eventID).Scan(&count)
	} else {
		err = DB.QueryRow("SELECT COUNT(*) FROM active WHERE event_id = $1", eventID).Scan(&count)
	}
	if err != nil {
...
func GetFestivalEventEntriesChunk(festivalID int64, eventID int64, chunk int64, archived bool) ([][]string, bool, error) {
	...
	rows, err := DB.Query(query, eventID, CHUNKSIZE, offset)
	defer func() {
		if rows != nil {
			rows.Close()
		}
...
func GetArchivedEventsIdsTimes(festivalId int64) ([]int, []int, error) {
	...
	rows, err := DB.Query("SELECT id, last_used_at FROM event WHERE active = FALSE AND festival_id = $1 ORDER BY id DESC", festivalId)
	if err != nil {
...
func ChangeCurrentEventMax(festivalCode string, newMax int) error {
	...
	time := time.Now().Unix()
	_, err = DB.Exec("INSERT INTO gauge_max (gauge_max, time, event_id) VALUES ($1, $2, $3)", newMax, time, eventId)
	if err != nil {
