...
func GetFestival(festivalCode string) (*models.FestivalData, error) {
	...
	err := DB.QueryRow("SELECT id, created_at, pin, password, code FROM festival WHERE code = ?", festivalCode).Scan(&festival.Id, &festival.CreatedAt, &festival.Pin, &festival.PasswordHash, &festival.Code)
	if err != nil {
...
func CreateRefreshToken(token models.RefreshToken) error {
	_, err := DB.Exec("INSERT INTO refresh_token (token, expires_at, last_used_at, user_id, revoked) VALUES (? ,? ,? ,? ,? )", token.Token, token.ExpiresAt, token.UserId, token.Revoked)
	return err
}
...
func UpdateRefreshToken(token models.RefreshToken) error {
	_, err := DB.Exec("UPDATE refresh_token SET expires_at = ?, last_used_at = ?, revoked = ? WHERE id = ?", token.ExpiresAt, token.Revoked, token.Id)
	return err
}
...
func CreateFestival(festival models.FestivalData) (int, error) {
	result, err := DB.Exec("INSERT INTO festival (code, password, pin, created_at, expires_at) VALUES (?, ?, ?, ?, ?)", festival.Code, festival.PasswordHash, festival.Pin, festival.CreatedAt)
	if err != nil {
		return -1, err
	}
